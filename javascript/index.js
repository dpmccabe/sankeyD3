...
import {scale, scaleOrdinal} from "d3-scale";
import {rgb} from "d3-color";
import {drag} from "d3-drag";
import {zoom} from "d3-zoom";
import {values} from "d3-collection";
import {axisBottom} from "d3-axis";
import {default as sankey} from "./lib/d3-sankey/src/sankey.js";
...
HTMLWidgets.widget({
    ...
    renderValue: function(el, x, instance) {
        ...
        var d3Zoom = zoom().scaleExtent([.75, 3]);    
        ...
        var format_use = function(d) { 
          if (options.numberFormat == "pavian") {
            var formated_num;
            if (d > 1000) {
              formated_num = format(".3s")(d);
            } else if (d > 0.001) {
              formated_num = format(".3r")(d);
            } else {
              formated_num = format(".3g")(d);
            }
            ...
            formated_num = format(options.numberFormat)(d);
          }
          return formated_num;
        }
        ...
        var max_posX = max(sankey.nodes(), function(d) { return d.posX; });
        ...
        function dragmove(d) {
          if (options.dragX || options.dragY) {
            select(this).attr("transform", 
                    "translate(" + 
                      (options.dragX? (d.x = Math.max(0, event.x)) : d.x ) + "," +
                      (options.dragY? (d.y = Math.max(0, event.y)) : d.y ) + ")");
            sankey.relayout();
        ...
        if (options.zoom) {
          d3Zoom.on("zoom", redraw)
          function redraw() {
            select(el).select(".zoom-layer").attr("transform",
          ...
          select(el).select("svg").on("dblclick.zoom", null);
        ...
        var path_filter = function(path, path1) { 
        ...
        var newNode = node.enter().append("g")
            ...
            .on("dblclick", function(d) {
               ...
               if (d.inactive) {
                 select(this).selectAll('rect').style("stroke-width",options.nodeStrokeWidth+2)
                 select(this).selectAll('text').style('font-weight', 'bold');
               } else {
                 select(this).selectAll('rect').style("stroke-width",options.nodeStrokeWidth)
                 select(this).selectAll('text').style('font-weight', 'normal');
               }
        ...
        node
            ...
            .style("fill", function(d) {
                if (d.targetLinks[0]) {
                  ...
                  return d.color = rgb(d.targetLinks[0].source.color).darker(1); 
                } else {
                  return d.color = color_node(d); 
                }
                })
            .style("stroke", function(d) { return rgb(d.color).darker(2); })
        ...
        if (options.xAxisDomain) {
          ...
          var x = scaleOrdinal().domain(options.xAxisDomain).range(axisXPos);
